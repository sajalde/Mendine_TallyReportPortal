...
public class GetInvoiceWebService : System.Web.Services.WebService {
    ...
    public string[] GetInvoice(string searchTerm)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("procINVOICESEARCH", con);
            cmd.CommandType = CommandType.StoredProcedure;

            SqlParameter parameter = new SqlParameter("@INVOICE", searchTerm);
            ...
            while (rdr.Read())
            {
               ...
               // interviewerName.Add(string.Format("{0}-{1}", rdr["empName"], rdr["empCode"].ToString()));
                invoice.Add(string.Format("{0}", rdr["invoiceno"].ToString()));
            }
        }
    ...
    public string[] Getcustomer(string searchTerm)
    {
        ...
        using (SqlConnection con = new SqlConnection(cs))
        {
            ...
            while (rdr.Read())
            {
                ...
                //interviewerName.Add(rdr["empName"].ToString());
                // interviewerName.Add(string.Format("{0}-{1}", rdr["empName"], rdr["empCode"].ToString()));
                customer.Add(string.Format("{0}", rdr["customer"].ToString()));
            }
        }
