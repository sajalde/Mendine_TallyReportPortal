...
public partial class OnlineReport_GodownStockTransfer : System.Web.UI.Page
{
    ...
    {
        string ComboName = "StockCategory,StockItem,SourceGodown,DestinationGodown";
    ...
    private void GenerateRDLCReport(Report_Search repParamSearch)
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/rdlcs/Stock/Report_GodownStockTransfer.rdlc");

        DataSet dt = (new Report_DL()).BuildReportData_GodownStockTransfer(repParamSearch);
        if (dt.Tables.Count >= 1)
        {
            ReportViewer1.LocalReport.DataSources.Clear();
            ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource()
            {
                Name = "dsGodownStockTransfer",
                Value = dt.Tables[0]
            });
        }
