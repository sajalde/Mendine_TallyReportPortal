...
public partial class OnlineReport_PurchaseRegister : System.Web.UI.Page
{
    ...
    {
        string ComboName = "VendorName,StockGroup,StockItem, GodownName";
    ...
    private void GenerateRDLCReport(Report_Search repParamSearch)
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/rdlcs/Purchase/Report_PurchaseRegister.rdlc");

        DataSet dt = (new Report_DL()).BuildReportData_PurchaseRegister(repParamSearch);
        if (dt.Tables.Count >= 1)
        {
            ReportViewer1.LocalReport.DataSources.Clear();
            ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource()
            {
                Name = "dsPurchaseRegister",
                Value = dt.Tables[0]
            });
        }
    ...
    protected void ExportCSVReport()
    {
        ...
        Response.AppendHeader("Content-Disposition", "attachment; filename=Purchase Register." + extension);
