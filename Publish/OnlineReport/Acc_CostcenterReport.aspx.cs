...
public partial class OnlineReport_Acc_CostcenterReport : System.Web.UI.Page
{
    ...
    {
        string ComboName = "CostCenter,LedgerName";
    ...
    private void GenerateRDLCReport(Report_Search repParamSearch)
    {
        ...
        ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/rdlcs/Accounts/Report_CostCenterReport.rdlc");

        DataSet dt = (new Report_DL()).BuildReportData_CostCenterReport(repParamSearch);
        if (dt.Tables.Count >= 1)
        {
            ReportViewer1.LocalReport.DataSources.Clear();
            ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource()
            {
                Name = "dsCostCenterReport",
                Value = dt.Tables[0]
            });
        }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            //--- Cost center Name::  Multi Select List Box Values  Item--
            string strCostCenterName = string.Empty;
            foreach (ListItem item in lbCostCenter.Items)
            {
                if (item.Selected)
                {
                    strCostCenterName += "'" + item.Text + "'";
                    strCostCenterName += ",";
                }
            }
    ...
    protected void ExportCSVReport()
    {
        ...
        Response.AppendHeader("Content-Disposition", "attachment; filename=Cost Center Report." + extension);
